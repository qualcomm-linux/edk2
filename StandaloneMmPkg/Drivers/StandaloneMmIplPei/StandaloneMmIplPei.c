  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
    Size   = 0;
    Status = MmAccess->GetCapabilities ((EFI_PEI_SERVICES **)PeiServices, MmAccess, &Size, NULL);
    ...
    for (Index = 0; Index < MmramRangeCount; Index++) {
      Status = MmAccess->Open ((EFI_PEI_SERVICES **)PeiServices, MmAccess, Index);
      if (EFI_ERROR (Status)) {
...
Done:
  if (MmAccess != NULL) {
    ...
    //
    for (Index = 0; Index < MmramRangeCount; Index++) {
      Status = MmAccess->Close ((EFI_PEI_SERVICES **)PeiServices, MmAccess, Index);
      if (EFI_ERROR (Status)) {
    ...
    //
    for (Index = 0; Index < MmramRangeCount; Index++) {
      Status = MmAccess->Lock ((EFI_PEI_SERVICES **)PeiServices, MmAccess, Index);
      if (EFI_ERROR (Status)) {
        //
